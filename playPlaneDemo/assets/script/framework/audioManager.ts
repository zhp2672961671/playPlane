...
export class AudioManager {
     ...
     */
    playMusic (name: string, loop: boolean, volume?:number) {
        ...
        ResourceUtil.loadRes(path, AudioClip, (err: any, clip: any) => {
            ...
            else { source.volume = this.musicVolume }
            source.loop = loop;
            source.play();
        });
    }
     ...
     * @param {Boolean} loop 是否循环播放
     */
    playSound (name: string, loop: boolean = false, volume?: number) {
        if (!this.soundVolume) {
            return;
        }
        if (!GameManager.isMusic) {
        ...
        ResourceUtil.loadRes(path + name, AudioClip, (err: any, clip: any) => {
            ...
            else { source.volume = this.soundVolume }
            source.loop = loop;
            source.play();
    ...
    setBg () {
        for (const i in this.audios) {
            if (this.audios.hasOwnProperty(i)) {
                ...
                if (audio.source.clip.name == Constant.AUDIO_MUSIC.BG) {
                    let audioBg = audio.source;
                    return audioBg.playing;
                }
            }
        }
